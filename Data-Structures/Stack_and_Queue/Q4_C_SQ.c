...
void reverse(Queue *q) {
	// 함수 시작 시, 스택이 비어있지 않다면 스택을 비운다.
	// -> 스택을 인자로 받은 게 아니기 때문에 새로운 스택을 만들어야 한다.
	Stack s;
	s.ll.head = NULL;
	s.ll.size = 0;
	s.ll.tail = NULL;
	...
	// 큐가 1, 2, 3, 4, 5 라면
	// 큐에서 dequeue 하면 1, 2, 3, 4, 5 순으로 꺼내지기 때문에
	// 이를 스택에 push 하면 1, 2, 3 ,4, 5 순으로 들어가게 된다.
	while (isEmptyQueue(q) == 0) {
		int item = dequeue(q);
		push(&s, item);
	}

	// 이제 스택에서 pop 하면 역순으로 5, 4, 3, 2, 1 로 나온다.
	// 즉, 스택에서 pop 해서 그 순서대로 enqueue 하면 된다.
